<template>
	<div class="nav-menu">
		...
		<div>
			<a href="#" @click.prevent="TOGGLE_MODAL_CREATE_POST">
				<svg
		...
		<div>
			<div class="nav-menu-avatar">
				...
				<div v-if="userActionMoreShow" class="user-action-more">
					<ul class="userActionMoreShow">
						<li>
							<router-link :to="{name: 'profileUser', params: {id: $store.getters.user._id}}" @click="toggleUserMore">
								<span>
									<svg
...
export default {
	created() {
		window.onclick = (e) => {
			if (e.target.closest('.nav-user-avatar')) {
				this.toggleUserMore()
				return
			}

			if (!e.target.closest('.userActionMoreShow'))
				this.userActionMoreShow = false
		}
	},
	methods: {
		...mapMutations(['TOGGLE_MODAL_CREATE_POST']),
		toggleUserMore() {
			this.userActionMoreShow = !this.userActionMoreShow
		},
	},
	data() {
		return {
			userActionMoreShow: false,
		}
	},
}
</script>
